...
from audit_logger import log_action, purge_old_logs, get_weekly_summary, build_markdown_section  # noqa: E402
...
def run_skill(reference_date: datetime | None = None, dry_run: bool = False) -> str:
    ...
    try:
        ...
        log_action("skill_error", "Skill8_WeeklyAuditBriefer",
...
def _run_skill_inner(reference_date: datetime | None = None, dry_run: bool = False) -> str:
    ...
    log_action("skill_start", "Skill8_WeeklyAuditBriefer",
               parameters={"week_end": _date_str(week_end),
                            "week_start": _date_str(week_start),
                            "dry_run": dry_run})

    # Purge audit logs older than 90 days (retention policy)
    deleted = purge_old_logs(days=90)
    if deleted:
        log(f"Audit log purge: deleted {len(deleted)} old file(s)")
    ...
    # Step 4b: Collect audit log summary
    log("Step 4b: Collecting audit log summary (last 7 days)...")
    audit_summary = get_weekly_summary(days=7)
    audit_md = build_markdown_section(audit_summary)
    log(f"  Audit summary: {audit_summary.get('total_actions', 0)} action(s) across "
        f"{len(audit_summary.get('by_actor', {}))} actor(s)")
    print()
    ...
    period_str = f"{_date_str(week_start)} to {_date_str(week_end)}"
    ...
    log_action(
        "briefing_written", "Skill8_WeeklyAuditBriefer",
        target=str(output_path),
        parameters={"period": period_str, "dry_run": dry_run,
                    "audit_actions": audit_summary.get("total_actions", 0)},
        approval_status="n/a",
    ...
    log_action(
        "skill_complete", "Skill8_WeeklyAuditBriefer",
        target=str(output_path),
        parameters={"dry_run": dry_run, "done_items": len(tasks),
                    "bottlenecks": bottleneck_count,
                    "revenue": revenue_total, "expenses": expense_total},
        result="success",
    )
